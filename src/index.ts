...
const dummyDataSqls = fs.readFileSync('./sql/sampleData.sql').toString().split(';').slice(0, -1)
...
const db = new sqlite.Database('./data/data.db', (err) => {
  ...
  db.run('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, userID TEXT NOT NULL);', (err) => {
    if (err) {
      console.error(err);
...
app.get("/user/:userID", (req:express.Request, res:express.Response)=>{
  ...
  db.run('INSERT INTO users (userID) Values (?);',[req.params.userID],(err) =>{
    if(err){
      console.error(err)
